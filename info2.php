...
$nameP = "info2";
$query1 = "SELECT nome,ingredienti,tipo,prezzo,qty FROM pizze where qty > 0";
...
<body>
 ...
 <table class="tabellaInfo">
   <tr><th>Nome</th><th>Ingredienti</th><th>Tipo</th><th>Prezzo</th><th>qty</th></tr>
   <?php 
    while($ok= mysqli_fetch_assoc($result)){
     $p=$ok['prezzo']/100;
     $p=number_format($p,2).' €';
      echo "<tr><td>$ok[nome]</td><td>$ok[ingredienti]</td><td>$ok[tipo]</td><td>$p</td><td>$ok[qty]</td></tr>";
     }
   ?>
  </table>
  </div>
